...
[TestClass]
public class ReservationExporter_CSVExportStrategy
{
    ...
    [TestMethod]
    public void ExportReservations_WithCSVExportStrategy_ShouldSetHeader()
    {
        var reservations = _reservationRepo.GetAll();
        var exportStrategy = new CSVExportStrategy();
        ...
        Assert.IsTrue(result.StartsWith("data:text/csv;base64,"));
    }

    [TestMethod]
    public void ExportReservations_WithCSVExportStrategy_ShouldSetContent()
    {
        ...
        var reservationExporter = new ReservationExporter(exportStrategy);
        var result = reservationExporter.ExportReservations(reservations);
        var expected = $"data:text/csv;base64,{Convert.ToBase64String(Encoding.UTF8.GetBytes($"DEPOSITO,RESERVA,PAGO\n{warehouse.WarehouseId},{reservation.ReservationId},nulo\n"))}";
